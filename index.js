const MinecraftColorCodes = {
    ...
    obfuscate: function (string, elem) {
        ...
        function init(el, str) {
            ...
            MinecraftColorCodes.obfuscators.push(window.setInterval(function () {
    ...
    applyCode: function(string, codes) {
        ...
        for (let code of codes) {
            elem.style.cssText += MinecraftColorCodes.styleMap[code] + ";";
            if (code === "§k") {
                MinecraftColorCodes.obfuscate(string, elem);
                obfuscated = true;
            }
        }
    ...
    parseStyle: function (string) {
        ...
        if (indices[0] !== 0) final.appendChild(MinecraftColorCodes.applyCode(string.substring(0, indices[0]), []));
    ...
    clearObfuscators: function () {
        for (let obfuscator of MinecraftColorCodes.obfuscators) clearInterval(obfuscator);
        MinecraftColorCodes.obfuscators = [];
    }
}

String.prototype.replaceColorCodes = function() {
  MinecraftColorCodes.clearObfuscators();
  return MinecraftColorCodes.parseStyle(String(this));
};